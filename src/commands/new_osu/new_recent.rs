...
async fn new_recent(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let raw_user = if let Ok(x) = args.single_quoted() {
        ...
        if let Some(db_data) = db_data {
            debug!("Got user from database.");
            instant_recent = db_data.instant_recent;
            db_data.osu_id.to_string()
        } else {
            debug!("Got user from nick or username.");
            msg.member(ctx).await?.display_name().to_string()
        }
    };
    ...
    let res_recent_data = if !instant_recent {
        client_lock
            .read()
            .await
            .get(&format!(
        ...
        client_lock
            ...
            .await
            .get(&format!(
                "https://osu.ppy.sh/api/v2/users/{}/scores/recent?mode=osu&include_fails=1&limit=1",
                &user
            ))
            .send()
            .await?
